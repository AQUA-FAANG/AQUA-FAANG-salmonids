...
# Make sample design csv files for rainbow trout BodyMap samples, to run nf-core pipelines
...
decodeSampleID <- function (id) {
	...
	fullName <- paste0(
		...
		tissue #,
		# ifelse(rep %in% c("A", "B"), paste0("_", rep), "")
	)
	group <- paste0(
		species, "_",
...
data <- "/mnt/labfiles/Aqua-Faang/"
results <- "/mnt/project/Aqua-Faang/nfcore"

# ChIP samples
inPath = paste0(data, "/RainbowTrout/ChIP")
outPath = paste0(results, "/RainbowTrout/BodyMap/ChIP")
...
design <- tibble(sample = unique(sub("_[12]\\.fq\\.gz$", "", list.files(inPath, ".*fq.gz")))) %>%
	...
	mutate(control = sapply(group, function(x) sub("_[AB]$", "", sub(sub("_.*", "", sub(".*_ChIP-", "", x)), "Input", x)))) %>%
...
# ATAC samples
inPath = paste0(data, "/RainbowTrout/ATAC")
outPath = paste0(results, "/RainbowTrout/BodyMap/ATAC")
if (!dir.exists(outPath)) {
	dir.create(outPath)
}
design <- tibble(sample = unique(sub("_[12]\\.fq\\.gz$", "", list.files(inPath, ".*fq.gz")))) %>%
...
# RNA samples
inPath = paste0(data, "/RainbowTrout/RNA")
outPath = paste0(results, "/RainbowTrout/BodyMap/RNA")
if (!dir.exists(outPath)) {
	dir.create(outPath)
}
design <- tibble(sample = unique(sub("_[12]\\.fq\\.gz$", "", list.files(inPath, ".*fq.gz")))) %>%
